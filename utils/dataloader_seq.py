"""
    Dataloader Seq -  Dataloader for the Sequential model (LSTM classifier).
...
class DataLoaderSeq(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(seq):
            ...
            label_2 = self.labels_array[id_s+i][1]

            for j in range(len(self.classes_speed)):
                if label_1 == self.classes_speed[j]:
                    id_1 = j
                if label_2 == self.classes_steering[j]:
                    id_2 = j
